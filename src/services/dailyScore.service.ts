...
//       try {
//         // Idempotency check backed by unique index on DailyScoreHistory (memberId, moduleName, date)
//         const historyReward = new DailyScoreHistory();
//         historyReward.memberId = member._id;
//         historyReward.moduleName = "DailyScoreReward";
//         historyReward.score = 10;
//         historyReward.date = todayStr;
//         historyReward.referenceId = new ObjectId();

//         try {
//           await this.dailyScoreHistoryRepo.save(historyReward);
//         } catch (dupErr: any) {
//           if (dupErr.code === 11000) {
//             console.log(`[DailyScore] Member ${member._id} already rewarded for ${todayStr}. Skipping duplicate award.`);
//             continue;
//           }
//           throw dupErr;
//         }

//         console.log(`[DailyScore] Rewarding member ${member._id} (score: ${member.dailyScore}) with 10 total points for ${todayStr}...`);

//         // Award 10 points atomically to memberPoints and member
...
//           { $inc: { points: 10 } }
//         );
//         const memberPoints = await this.memberPointsRepo.findOneBy({ memberId: member._id });

//         // Create PointHistory entry
//         const history = new PointHistory();
//         history.memberId = member._id;
//         history.moduleName = "DailyScoreReward";
//         history.referenceId = historyReward.referenceId;
//         history.actionType = "reward";
//         history.type = "earned";
//         history.points = 10;
//         history.balanceAfter = memberPoints ? memberPoints.totalPoints : 10;
//         await this.historyRepo.save(history);

//         console.log(`[DailyScore] Awarded 10 points to member ${member._id}. New balance: ${memberPoints ? memberPoints.totalPoints : 10}`);
//       } catch (error: any) {
